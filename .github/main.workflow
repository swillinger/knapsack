...
action "deploy:docs2" {
  ...
  secrets = ["ZEIT_TOKEN"]
  #args = "deploy --platform-version 2 docs-site/build/bedrock --name bedrock-docs --meta GITHUB_SHA=$GITHUB_SHA --scope=basalt --target staging"
  runs = ["sh", "-c", "now deploy --platform-version 2 docs-site/build/bedrock --name bedrock-docs --token=$ZEIT_TOKEN --meta GITHUB_SHA=$GITHUB_SHA --scope=basalt --target production && now alias bedrock-docs --scope=basalt --name bedrock-docs --token=$ZEIT_TOKEN"]
}
