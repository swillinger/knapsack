...
## Site Settings
This edit form is accessible via the link in the footer, titled <a href="http://localhost:3999/settings" target="_blank">Site Settings</a>.
...
* Logo
    * The `Logo` field is going to take a URI or an image file and appear as an icon button.
* Title
...
2) Next you will add your custom pages within the new Custom Section you just added. Click on the `+` icon under `Custom Pages` and input your names to `Page Title` and `Page ID` just as before. (Example: `Page 1` and `page-one`) Those pages will appear in the left sidebar under the heading for the new Custom Section.
